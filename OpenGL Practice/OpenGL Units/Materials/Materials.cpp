...
int main(void)
{
	...
	* */
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0); // Format position data. Start at 0, have 3 data per points, stride by 5 * size of a float, void
	glEnableVertexAttribArray(0); //Enable the verticies to be inputed into the vertex shader location = 0


	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));  // Format normal data. Start at 6, have 3 data per points, stride by 6 * size of a float, void
	...
	while (!glfwWindowShouldClose(window)) //If we haven't closed the window
	{
		...
		mat4 model =  translate(model, cubePositions[0]) *rotate(model, float(glfwGetTime()), vec3(1.0f, 1.0f, 0.0f))* scale(model, vec3(200.0f));
		...
		ApplyMaterials("materialData.txt", shader);
		lightShader.use();
		...
		unsigned int lightPosLocation = glGetUniformLocation(lightShader.ID, "lightPos");
		glUniform3fv(lightPosLocation, 1, value_ptr(lightPosition));
