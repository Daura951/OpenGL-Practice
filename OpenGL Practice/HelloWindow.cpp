...
int main(void)
{
	...
	* */
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));  // Format normal data. Start at 6, have 2 data per points, stride by 8 * size of a float, void
	...
	if (isUsingTexture)
	{
		...
		unsigned char* imageData = stbi_load("container2Diffuse.png", &width, &height, &nrChannels, 0); //load diffuse map texture image
		if (imageData)
		{
		...
		imageData = stbi_load("container2_specular.png", &width, &height, &nrChannels, 0); //load specular map texture image
		if (imageData)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData); //generate the texture with image
			glGenerateMipmap(GL_TEXTURE_2D);
		...
		shader.use();
		glUniform1i(glGetUniformLocation(shader.ID, "material.diffuse"), 0); //Set uniform for diffuse sampler
		shader.setInt("material.specular", 1); //Set uniform for specular sampler
	}
	else shader.use();
	...
	while (!glfwWindowShouldClose(window)) //If we haven't closed the window
	{
		...
		ApplyLightParameters(NULL, shader);
		lightShader.use();
...
void ApplyLightParameters(const char* materialFilePath, Shader shader)
{
	if (materialFilePath != NULL)
	{
		...
		//DIRECTIONAL
		location = glGetUniformLocation(shader.ID, "dirLight.direction");
		glUniform3fv(location, 1, value_ptr(vec3(-0.2f, -1.0f, -0.3f)));

		location = glGetUniformLocation(shader.ID, "dirLight.ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "dirLight.diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(0.4f, 0.4f, 0.4f)));

		location = glGetUniformLocation(shader.ID, "dirLight.specular");
		glUniform3fv(location, 1, value_ptr(vec3(0.5f, 0.5f, 0.5f)));



		//POINT LIGHT 1
		location = glGetUniformLocation(shader.ID, "pointLights[0].position");
		glUniform3fv(location, 1, value_ptr(vec3(0.7f, 0.2f, 2.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[0].ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[0].diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[0].specular");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[0].constant");
		glUniform1f(location, 1.0f);

		location = glGetUniformLocation(shader.ID, "pointLights[0].linear");
		glUniform1f(location, 0.09f);

		location = glGetUniformLocation(shader.ID, "pointLights[0].quadratic");
		glUniform1f(location, 0.032f);



		//POINT LIGHT 2
		location = glGetUniformLocation(shader.ID, "pointLights[1].position");
		glUniform3fv(location, 1, value_ptr(vec3(2.3f, -3.3f, -4.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[1].ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[1].diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[1].specular");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[1].constant");
		glUniform1f(location, 1.0f);

		location = glGetUniformLocation(shader.ID, "pointLights[1].linear");
		glUniform1f(location, 0.09f);

		location = glGetUniformLocation(shader.ID, "pointLights[1].quadratic");
		glUniform1f(location, 0.032f);
		...
		//POINT LIGHT 3
		location = glGetUniformLocation(shader.ID, "pointLights[2].position");
		glUniform3fv(location, 1, value_ptr(vec3(-4.0f, 2.0f, -12.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[2].ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[2].diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[2].specular");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[2].constant");
		glUniform1f(location, 1.0f);

		location = glGetUniformLocation(shader.ID, "pointLights[2].linear");
		glUniform1f(location, 0.09f);

		location = glGetUniformLocation(shader.ID, "pointLights[2].quadratic");
		glUniform1f(location, 0.032f);
		...
		//POINT LIGHT 4
		location = glGetUniformLocation(shader.ID, "pointLights[3].position");
		glUniform3fv(location, 1, value_ptr(vec3(0.0f, 0.0f, -3.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[3].ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[3].diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(0.05f, 0.05f, 0.05f)));

		location = glGetUniformLocation(shader.ID, "pointLights[3].specular");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));

		location = glGetUniformLocation(shader.ID, "pointLights[3].constant");
		glUniform1f(location, 1.0f);

		location = glGetUniformLocation(shader.ID, "pointLights[3].linear");
		glUniform1f(location, 0.09f);

		location = glGetUniformLocation(shader.ID, "pointLights[3].quadratic");
		glUniform1f(location, 0.032f);


		//SPOT LIGHT
		location = glGetUniformLocation(shader.ID, "spotLight.position");
		glUniform3fv(location, 1, value_ptr(camera.Position));

		location = glGetUniformLocation(shader.ID, "spotLight.direction");
		glUniform3fv(location, 1, value_ptr(camera.Front));

		location = glGetUniformLocation(shader.ID, "spotLight.ambient");
		glUniform3fv(location, 1, value_ptr(vec3(0.0f, 0.0f, 0.0f)));

		location = glGetUniformLocation(shader.ID, "spotLight.diffuse");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));


		location = glGetUniformLocation(shader.ID, "spotLight.specular");
		glUniform3fv(location, 1, value_ptr(vec3(1.0f, 1.0f, 1.0f)));

		location = glGetUniformLocation(shader.ID, "spotLight.constant");
		glUniform1f(location, 1.0f);

		location = glGetUniformLocation(shader.ID, "spotLight.linear");
		glUniform1f(location, 0.09f);

		location = glGetUniformLocation(shader.ID, "spotLight.quadratic");
		glUniform1f(location, 0.032f);

		location = glGetUniformLocation(shader.ID, "spotLight.cutoff");
		glUniform1f(location, glm::cos(radians(12.5f)));

		location = glGetUniformLocation(shader.ID, "spotLight.outerCutoff");
		glUniform1f(location, glm::cos(radians(15.0f)));
