...
void main()
{
    ...
    float theta = dot(lightDirection, normalize(-light.direction)); //-light.direction due to right handed system on Z axis
    float epsilon = (light.cutoff - light.outerCutoff);
    float intensity = clamp((theta - light.outerCutoff) / epsilon, 0.0, 1.0); //Clamp leads to out attenuation

     diffuseLight *= intensity;
     specularLight *= intensity;

     //We have to attenuate our flashlight to make it realistic!!
